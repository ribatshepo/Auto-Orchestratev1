...
- **`install-component.sh` — targeted single-component installer** — New repo-root script `install-component.sh <skill|agent> <name>` deploys one newly-scaffolded component into an existing `~/.claude/` install without the full wipe-and-recopy `install.sh` performs. It copies just that component, re-syncs `manifest.json` + the prose docs `extend.py` touched (`agents/README.md`, `ARCHITECTURE.md`), backs up every overwritten file to a timestamped `backup-*/` dir, validates the installed manifest (rolling back on failure), and refuses on a missing install / missing-or-unregistered component. `--source`/`--target` are overridable; `--dry-run` previews. Completes the `extend.py … → install-component.sh …` workflow.
...
- **Documentation sync to current state** — New `docs/EXTENDING.md` (extension guide) and `docs/token-consumption-granularity.md`. README, ARCHITECTURE, INTEGRATION, CONTRIBUTING, RELEASE-NOTES, and PERMISSION-MODES reconciled with the codebase: agent count corrected to **22**, checkpoint schema to **1.10.0**, artifact contract to **145 rules + 7 consistency checks**, lib inventory updated (`_store_io.py`, `_time`, `DomainIndexer`, `agent-preamble.md`, `spawn-core.md`, `CONSTRAINTS-REGISTRY.md`), and `extend.py` / `install-component.sh` documented as the extension + deploy path. `manifest.json` `updated_at` bumped to 2026-06-13.
