...
| CONTEXT-DIET-001 | **Digest-by-default artifact reading** — When `checkpoint.optimizations.artifact_excerpt == true` (default for fresh installs), producers populate `envelope.excerpt` (≤600 chars) + `excerpt_pointers` via `build_envelope(...)`, the orchestrator enriches each `MANIFEST.jsonl` line with `artifact_type`/`status`/`excerpt`/`excerpt_pointers`, and consumers read the `MANIFEST.jsonl` index + excerpts first, deep-reading a full body ONLY when the task needs detail (logging `[CONTEXT-DIET-001] deep-read <path>`). Full bodies stay byte-identical on disk; legacy lines without `excerpt` fall back to full-body reads. Defined in `_shared/protocols/output-standard.md` §8. Token saving: avoids re-reading N full artifacts when an index scan suffices. |
| DOMAIN-QUERY-001 | **FTS retrieval over raw-JSONL scans** — Agents query the domain-memory SQLite index via the single entry point `DomainIndexer.query(store, text, limit)` (≤`limit` ranked rows) instead of reading whole `.orchestrate/domain/*.jsonl` ledgers. The index is a rebuildable derived artifact (`rebuild_index()` at boot); on any error, agents fall back to reading the JSONL source so no knowledge is lost. Defined in `agents/orchestrator.md` (Reading domain memory). Token saving: grows with ledger size across sessions. |
| PROTOCOL-PACK-SLIM-001 | **Slim spawn-protocol pack** — When `checkpoint.optimizations.protocol_pack_slim == true` (default for fresh installs), the spawn-prompt builder injects `_shared/protocols/spawn-core.md` (~2k tok: every OUT-/READ-/EARLY-/MAN-/PREAMBLE-/NAME-/ENG-STD constraint by ID + a Reference Index) instead of the full 5-doc stack (~7.5k); code-producing agents at Stage 3 additionally receive the full `engineering-standards.md` (enforcement preserved). The five source docs are unchanged on disk and read on demand. Safe fallback: missing `spawn-core.md` → full stack + `[PROTOCOL-PACK-SLIM-001 FAIL]`; a spawn may set `needs_full_protocols: true` to re-fatten. Token saving: ~5.5k tokens per non-code spawn × 20–60 spawns/session. |
| CONTINUITY-TIER-001 | **Tiered continuity brief** — When `checkpoint.optimizations.continuity_brief_tiered == true` (default for fresh installs), `continuity-scout` emits a `## HOT` core + `## Slice Index` over the same canonical `continuity-brief.md` (CONT-002/004/008 unchanged). Per PREAMBLE-001, spawns read HOT + their stage/scope slice by default (targeted offset reads) and deep-read the full brief on demand. PREAMBLE-002 stays sound: an agent must full-read the brief before declaring "(no relevant continuity item)". Legacy briefs without a Slice Index fall through to a full read; a spawn may set `needs_full_brief: true`. Defined in `agents/continuity-scout.md` + `_shared/protocols/agent-preamble.md`. Token saving: ~50–250 brief lines per spawn. |
