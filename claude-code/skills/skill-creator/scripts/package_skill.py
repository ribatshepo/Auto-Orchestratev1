...
def _print_usage(exit_code: int = EXIT_INVALID_ARGS) -> None:
    print("Usage: package_skill.py <path/to/skill-folder> [output-directory]")
    print("\nPackages a skill folder into a distributable .skill file (zip archive).")
    print("\nArguments:")
    print("  <skill-folder>     Path to the skill directory containing SKILL.md")
    print("  [output-directory] Optional destination directory (defaults to current directory)")
    print("\nExamples:")
...
def main():
    if len(sys.argv) >= 2 and sys.argv[1] in ("-h", "--help"):
        _print_usage(EXIT_SUCCESS)
    if len(sys.argv) < 2:
        _print_usage(EXIT_INVALID_ARGS)
